    ...
    vector<vector<int>> modifiedGraphEdges(int n, vector<vector<int>>& edges, int src, int dest, int target) 
    {
       ...
       if(dist[dest]==target)
       {
        ...
        for(int i=0;i<edges.size();i++)
        {
            ...
            if(w==-1)
            { //if we come across a negative edge, make it equal to 1 and check for the condition
                edges[i][2]=1;
                dp[u].push_back({v,1});
                dp[v].push_back({u,1});
...
//Explanation: https://leetcode.com/problems/modify-graph-edge-weights/solutions/3548231/dijkstra-algorithm-approach-beats-80-easy-to-understand/, also refer notebook
